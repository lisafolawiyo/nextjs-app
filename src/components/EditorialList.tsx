...
const EditorialList = ({
  ...
  const featuredImage1 = mostRecentPost._embedded?.['wp:featuredmedia']?.[0]?.source_url;

  const formattedMostRecentDate = new Date(mostRecentPost.date).toLocaleDateString('en-GB', {
    day: '2-digit',
    month: 'short',
    year: 'numeric',
  });
      ...
      <section className="editorial-latest-section">
        <div className="section-wrapper editorial-latest-wrapper">
          ...
          <div className="latest-right">
            ...
            <p className='editorial-desc'>{stripOuterTags(mostRecentPost.excerpt.rendered)}</p>
            <a href={`/post/${mostRecentPost.slug}`} className="read-btn latest-read-btn">
